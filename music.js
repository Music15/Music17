...
setInterval(() => {
  http.get(`http://test7aza2.glitch.me/`); //تعديل مهم حط اسم المشروع
}, 280000);
...
const GUILDID = '731508694482813061'; //  تعديل مهم ايدي السيرفر  


//البرفكس
const prefix = "n!"; // تعديل مهم حط هنا البرفكس او مقدمة الامر
...
//تغير الحالة
const developers = ["533615354459586560"] ///تعديل مهم حط هنا ايدي اللي بدك يتحكمو بالحالة
...
let cmds = {
  ...
  repeat: { cmd: 'repeat', a: ['loop','re','تكرار'] },
...
const youtube = new YouTube("AIzaSyBlhdvxiYuA3Nx0J-G_rQDYtdKKDHgN3N8"); 
  ...
  if (message.content === prefix + 'join') {
    if (message.member.voiceChannel) {
      message.member.voiceChannel.join()
        .then(connection => { 
          message.reply('\`Been to Room\`');
        })
        .catch(console.log);
    } else {
      message.reply('\`You must log in to audio room\`');
    }
  }
});
...
client.on('message', message => {
  var helplist = `**:notes:  List of orders** 

\`Play\` : Play the song, add it to the list, or complete the song [p] 
\`Pause\` : Pause the song 
\`Resume\` : Complete the song
\`stop\` : To pause the song and the bot to leave the room
\`forceskip\` : To directly skip the song
\`Queue\` : Show menu 
\`skipto\` : To skip to the next song in the next music queue
\`Skip\` : Skip to the next song
\`Volume\` : changing the voice [vol] 
\`Nowplaying\` : Show what's playing now [np] 
\`repeat\` : Repeat the song
\`Leave\` : اExit the audio room
